import projectCsvImportService from '@/services/projects/csvImport/csvImport.service';
...
export const exportProjects = catchAsync(async (req: Request, res: Response) => {
  ...
  const { employer_id, status, search } = req.query;

  const filters: any = {};
  if (employer_id) filters.employer_id = employer_id as string;
