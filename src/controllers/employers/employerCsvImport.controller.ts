...
export const exportEmployers = catchAsync(async (req: Request, res: Response) => {
  ...
  const { is_active, is_verified, search } = req.query;
  ...
  const rows = employers.map((employer: any) => {
    return [
      ...
      employer.gst_number || '',
      employer.is_active ? 'Yes' : 'No',
      employer.is_verified ? 'Yes' : 'No',
      employer.created_at ? new Date(employer.created_at).toISOString().split('T')[0] : '',
    ].map(field => {
