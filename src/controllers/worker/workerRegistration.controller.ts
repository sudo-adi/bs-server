 ...
 */
export const registerWorker = catchAsync(async (req: Request, res: Response) => {
  const {
    phone: phoneField,
    mobile_number,
    first_name,
  ...
  if (!phone || !first_name) {
    throw new AppError('Mobile number and first name are required', 400);
  }
  ...
  res.status(201).json({
    ...
    data: {
      ...
      current_stage: 'new_registration',
    },
  });
});
...
export const checkRegistrationStatus = catchAsync(async (req: Request, res: Response) => {
  ...
  const currentStage = profile.stage_transitions[0]?.to_stage || 'new_registration';
