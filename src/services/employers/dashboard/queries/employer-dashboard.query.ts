...
export class EmployerDashboardQuery {
  /**
   * Get employer dashboard overview with projects and statistics
   */
  static async getDashboardOverview(employerId: string) {
    ...
    const stats = {
      ...
      active_projects: projects.filter((p) => p.status === ProjectStatus.ONGOING).length,
      allocated_projects: projects.filter((p) => p.status === ProjectStatus.WORKERS_SHARED).length,
      ...
      total_workers: projects.reduce(
        (sum, p) => sum + ((p as any).project_matched_profiles?.length || 0),
        0
      ),
   ...
   */
  static async getProjectDetails(employerId: string, projectId: string) {
    // Verify employer exists
    const employer = await prisma.employers.findUnique({
      where: { id: employerId, deleted_at: null },
    });
    ...
    const enrichedWorkers = ((project as any).project_matched_profiles || []).map(
      (matchedProfile: any) => {
   ...
   */
  static async getEmployerProjects(
    employerId: string,
    filters?: {
      status?: string;
      search?: string;
